-- Create hybrid case similarity search function
...
AS $$
  SELECT 
    ...
    (
      ...
      -- Liability percentage proximity (0.2 weight)
      0.2 * (100 - ABS(COALESCE(c.liab_pct::numeric, 100) - query_liab_pct)) / 100 +
      
      -- Policy bucket match (0.1 weight)
      0.1 * CASE 
        WHEN (
          CASE 
            WHEN COALESCE(c.pol_lim, '0')::text ~ '^[0-9,.$]+$' 
            THEN REPLACE(REPLACE(REPLACE(c.pol_lim, '$', ''), ',', ''), '.', '')::numeric
            ELSE 0 
          END > 500000 AND query_policy_bucket = 'high'
        ) OR (
          CASE 
            WHEN COALESCE(c.pol_lim, '0')::text ~ '^[0-9,.$]+$' 
            THEN REPLACE(REPLACE(REPLACE(c.pol_lim, '$', ''), ',', ''), '.', '')::numeric
            ELSE 0 
          END BETWEEN 100000 AND 500000 AND query_policy_bucket = 'mid'
        ) OR (
          CASE 
            WHEN COALESCE(c.pol_lim, '0')::text ~ '^[0-9,.$]+$' 
            THEN REPLACE(REPLACE(REPLACE(c.pol_lim, '$', ''), ',', ''), '.', '')::numeric
            ELSE 0 
          END < 100000 AND query_policy_bucket = 'low'
        ) THEN 1 ELSE 0 END +
